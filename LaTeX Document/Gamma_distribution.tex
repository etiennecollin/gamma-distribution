...
\begin{equation*}
	\forall^k_{i=1}\left[X_i\sim\E\text{xp}(\lambda)\right]\text{ and are i.i.d. random variables, then }Y=\sum^k_{i=1}\left[X_i\right]\sim\G\text{amma}(k, \lambda)
\end{equation*}
\begin{proof}
...
\begin{proof}
	...
	\end{equation*}
	\bigred{This does not prove the link with chi-squared distribution... as Z follows N(0,1)... right?}
	One may generalize this result by using that $\forall^n_{i=1}\left[Z_i\sim\N(0, 1)\right]$ and are i.i.d. to
	find the distribution of
	\begin{equation*}
		Y = \sum^n_{i=1}\left[Z^2_i\right]
	\end{equation*}
	Hence, using the moment generating function,
	\begin{equation}\label{eq:relation:chisquared:n}
		\begin{split}
			M_{\sum^n_{i=1}\left[Z^2_i\right]}(t)	&=	\Pi^n_{i=1}\left[M_{Z^2_i}(t)\right]\\
													&=	M(t)^n\\
