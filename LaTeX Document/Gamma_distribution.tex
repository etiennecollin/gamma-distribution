...
\newcommand{\docDueDate}{May 9, 2022}
...
\pagebreak
\subsection{Probability Density Function \bigred{INTRODUCE PDF}}\label{subsec:gamma:pdf}
...
distribution with parameters $\alpha=4$ and $\lambda=3$. As an example, this probability density could be used to
evaluate the time until four events that occur at a rate of three events per unit time take place:
\figurecenter{0.9}{Gamma_PDF.png}{Graph featuring the plot of the probability density function of a gamma distribution.}{fig:graph:gamma:pdf}
...
\figurecenter{1}{Gamma_CDF.png}{Graph featuring the plot of the cumulative distribution function of a gamma distribution.}{fig:graph:gamma:cdf}
...
$q(p)=F^{-1}(p)$, but $F^{-1}(x)$ is not defined, since $F(x)$ is an integral with no antiderivative.
...
\figurecenter{1}{Gamma_mode.png}{Graph featuring the probability density function of two distinct gamma distributions.}{fig:graph:gamma:mode}
...
$\G\text{amma}(5, 1)$.
\figurecenter{1}{Gamma_median_approximation.png}{Graph featuring the probability density function of a gamma distribution with parameters $\alpha=5$ and $\lambda=1$.}{fig:graph:gamma:median:approximation}
...
negative and fractional values of $\alpha$. Notice how the asymptotes of the graph are found at every negative integer value; the
...
\subsection{The Exponential Distribution \bigred{X+Y}}\label{subsec:specialcases:exponential}
...
it can also be said that $X\sim\E\text{xp}(\lambda)$. The continuous random varialbe $X$ thus follows an exponential distribution with rate parameter
...
\pagebreak
\subsubsection{Sum of Exponential Random Variables \bigred{add Introduction}}
...
\begin{equation}
	...
	\end{split}
\end{equation}
Finally, using the definition of the gamma notation, we arrive to
\begin{equation}
	\begin{split}
...
\begin{equation}
    ...
    \end{split}
\end{equation}
We will note here that the integral part of this equation corresponds to the Beta distribution function. Hence, we get
\begin{equation}
	    f_{X+Y} (z) = \frac{\lambda^{\alpha_1 + \alpha_2}}{\Gamma(\alpha_1) \Gamma(\alpha_2)}  e^{-\lambda z} z^{\alpha_1 + \alpha_2 - 1} B(\alpha_1, \alpha_2)\\
\end{equation}
Using an alternative definition of the beta distribution, which was proven in the last demonstration, we arrive to our final answer :
\begin{equation}
	    f_{X+Y}(z) = \frac{\lambda^{\alpha_1 + \alpha_2}}{\Gamma(\alpha_1 + \alpha_2)} e^{-\lambda z} z^{(\alpha_1 + \alpha_2) - 1}\\
\end{equation}
This corresponds, most clearly, to the probability density function of a gamma distribution.
\begin{equation}
    Z \sim Gamma({\alpha_1 + \alpha_2,  \lambda})
\end{equation}
...
\pagebreak
\subsection{The Beta Distribution \bigred{X/(X+Y)}}\label{subsec:specialcases:beta}
To introduce the beta distribution, let's show how it is related to the gamma distribution. Indeed, for
$X\sim\G\text{amma}(\alpha, \lambda)$ and $Y\sim\G\text{amma}(\beta, \lambda)$, then
$\frac{X}{X+Y}\sim\G\text{amma}(\alpha+\beta, \lambda)$ Let's prove it:
\begin{proof}
	\bigred{X/(X+Y)}
\end{proof}

It follows from the previous result that the beta distribution is, in fact, a proportion of two gamma distributions! In
this section, we will define this distribution more precisely and explore its distinguishing features.
...
Alternatively, using a change of variables,
...
which is clearly the Beta distribution.
...
\begin{equation}\label{eq:chisquared-density}
	...
	\end{split}
\end{equation}
\figurecenter{1}{Chi2_PDF.png}{Graph of the probability distribution function of a $\chi^2_n$ distribution for various values of $n$.}{fig:specialcases:chi2:pdf}
...
\end{equation}
The central limit theorem will not be proved as it is quite intuitive; the more data samples one has, the more
centered around the expected value the probability density function graph will be. That being said, here is a graph
...
\figurecenter{1}{Central_limit_theorem.png}{Graph showcasing the central limit theorem by comparing the probability density function graph of $\G\text{amma}(60, 1)$ to that of $\N(60, 60)$.}{fig:graph:clt}
...
\lstinputlisting[language=Python, caption={}, label=code:simulation:1]{_assets/Simulation_1.py}
...
\lstinputlisting[language=Python, caption={}, label=code:simulation:2]{_assets/Simulation_2.py}
