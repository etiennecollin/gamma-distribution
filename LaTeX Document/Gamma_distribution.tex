...
\bigred{graph, integral of pdf=1}
...
\subsection{Moment generating function \incomplete}
...
\pagebreak
\subsection{Variance \incomplete}
...
\bigred{Since we can generate the graph of the CDF, we should still explain what the quantiles are... at least visually using a graph}
...
\subsection{Median \bigred{proof}}
...
\subsection{Skewness \incomplete}
\begin{equation}
	\text{Skewness} = \frac{2}{\sqrt{\alpha}}
\end{equation}

\subsection{Excess kurtosis \incomplete}
\begin{equation}
	\text{Kurtosis} = \frac{6}{\alpha}
\end{equation}

\subsection{Information Entropy \incomplete}
\begin{equation}
	\text{Entropy} = \alpha + \ln{\lambda} + \ln{\Gamma(\alpha)} + (1-\alpha)\psi(\alpha)
\end{equation}

\subsection{Characteristic function \incomplete}
\begin{equation}
...
\pagebreak
\subsubsection{Special case of $\Gamma(\sfrac{1}{2})$ \bigred{Gamma12\_BNM\_VC.pdf}}
...
\pagebreak
\subsection{The Exponential Distribution \bigred{graph?, gamma\_sum}}
...
\pagebreak
\subsection{The Chi-Squared Distribution \bigred{graph}}\label{subsec:specialcases:chisquared}
...
As was shown previously in \autoref{subsec:specialcases:chisquared}, the normal distribution is related to the gamma
...
\pagebreak
\subsection{The Beta Distribution \bigred{X/(X+Y)}}
\bigred{start with proof of X/(X+Y)}
...
\pagebreak
\subsection{The Wishart Distribution \incomplete}
\bigred{what is $\Sigma$, what is $\Gamma_p(\alpha)$... explain}
...
where $\Gamma_p(\alpha)$, is the multivariate gamma function.
\begin{equation}
...
\subsection{Simulation 1 \bigred{add ref}}
...
\pagebreak
\subsection{Simulation 2 \bigred{add ref}}
