...
\begin{proof}
	...
	\begin{equation*}
		W=\sum^n_{i=1}X_i
	\end{equation*}
	where $W$ is a sum of $n$ $X$ i.i.d. random variables following the exponential distribution $\E\text{xp}(\lambda)$.
	...
	\begin{equation*}
		\begin{split}
			W						&=	\sum^k_{i=1}X_i\\
			M_{\sum^n_{i=1}X_i}(t)	&=	E(e^{t\sum^k_{i=1}X_i})\\
									&=	E(\prod^k_{i=1}e^{tX_i})\\
									&=	\prod^k_{i=1}E(e^{tX_i})\\
									&=	\prod^k_{i=1}M_{X_i}(t)\\
	...
	\begin{equation*}
		\begin{split}
W													&=	\sum^{k+1}_{i=1}X_i\\
													&=	\sum^{k}_{i=1}\left[X_i\right] + X_{k+1}\\
M_{\sum^{k}_{i=1}\left[X_i\right] + X_{k+1}}(t)		&=	E\left(e^{t\left(\sum^{k}_{i=1}\left[X_i\right] + X_{k+1}\right)}\right)\\
													&=	E\left(e^{t\left(\sum^{k}_{i=1}X_i\right)}e^{tX_{k+1}}\right)\\
													&=	E\left(e^{t\left(\sum^{k}_{i=1}X_i\right)}\right)E\left(e^{tX_{k+1}}\right)\\
													&=	M_{\sum^n_{i=1}X_i}(t)M(t)\\
													&=	\left(\frac{\lambda}{\lambda - t}\right)^k\left(\frac{\lambda}{\lambda - t}\right)\\
													&=	\left(\frac{\lambda}{\lambda - t}\right)^{k+1}
		\end{split}
	\end{equation*}
\end{proof}
