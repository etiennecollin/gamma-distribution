...
\bigred{INTRODUCE PDF}
\begin{equation}\label{eq:gamma:pdf}
	f(x) = \frac{\lambda^\alpha}{\Gamma(\alpha)}x^{\alpha-1}e^{-\lambda x}\text{ for }x\in (0,\,\infty)
\end{equation}

Here is a graph of the probability density function of a gamma distribution with parameters $\alpha=4$ and $\lambda=3$:
...
\end{equation}
The central limit theorem will not be proved as it is quite intuitive; the more data samples you have, the more
centered around the expected value the probability density function graph will be. That being mentioned, here is a graph
...
having $60$ data samples following a $\E\text{xp}(1)$ distribution which fits the central limit theorem. Here is the graph:
