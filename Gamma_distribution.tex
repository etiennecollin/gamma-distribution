...
\subsection{Chi-Squared Distribution}
	Then, the chi-squared distribution is a gamma distribution with parameters $\alpha=\sfrac{1}{2}$ and $\lambda=\sfrac{1}{2}$. This
	relationship may be proved using a change of variables with $Y=Z^2$ for $Y\sim\chi^2_1$.
	\begin{proof}
		\begin{equation}\label{eq:relation:chisquared}
			\begin{split}
				G(y)	&=	P(Y\leq y)\\
						&=	P(Z^2\leq y)\\
				...
				g(y)	&=	\frac{f(\sqrt{y})}{2\sqrt{y}}+\frac{f(-\sqrt{y})}{2\sqrt{y}}\\
						...
						&\equiv \G\text{ammma}(\sfrac{1}{2}, \sfrac{1}{2})
			\end{split}
		\end{equation}
	\end{proof}
